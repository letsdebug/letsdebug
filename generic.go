...
func (c caaChecker) Check(ctx *scanContext, domain string, method ValidationMethod) ([]Problem, error) {
	...
	if len(rrs) > 0 {
		...
		for _, r := range records {
			...
			if err != nil {
				probs = append(probs, caaPossiblyInvalid(domain, wildcard, r.Value, err))
			} else {
...
func caaPossiblyInvalid(domain string, wildcard bool, record string, problem error) Problem {
	return Problem{
		...
		Detail:      fmt.Sprintf("Problematic CAA record: %s, error: %s", record, problem),
